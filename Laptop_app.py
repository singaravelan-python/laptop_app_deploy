...
cpu = st.selectbox("Processer on the system",df['Cpu'].unique())
ram = st.radio("RAM on the system(in GB)",[4,8,12,16,24,32,64,128],index=1,horizontal=True)
gpu = st.selectbox("Graphics Card on laptop",df['Gpu'].unique(),index=1)
os = st.radio("Operating System",df['OpSys'].unique(),index=1)
weight = st.slider("Weight of the laptop(in kg)",min_value=0.7,max_value=4.8,value=2.0,step=0.1)
ips = st.radio("IPS Display?",["Yes","No"],index=1,horizontal=True)
touchscreem = st.radio("Touchscreen Display?",["Yes","No"],index=1,horizontal=True)
...
screen_resolution = st.selectbox("Screen Resolution of the laptop",
                                 ["2560x1600","1440x900","1920x1080","2880x1800","1366x768",
                                  ...
                                  "2160x1440","2560x1440","1600x900","2736x1824","2400x1600"],index=2)
cpu_speed = st.slider("Clock speed of CPU(in GHZ)",min_value=0.9,max_value=3.8,value=2.3,step=0.1)
hdd = st.selectbox("Hard Drive on the laptop(in GB, if SSD select 0)", [0,256,512,1024,2048,4096])
ssd = st.selectbox("SSD storage on laptop(in GB, if HDD select 0)", [0,128,256,512,1024,2048], index=3)
...
if st.button("PREDICT PRICE"):
    ...
    query = np.array([[company,typename,cpu,ram,gpu,os,weight,ips,
                       touchscreem,ppi,cpu_speed,hdd,ssd]])
    
    op = pipe.predict(query)
    st.subheader(f'''The predicted price of the laptop with the above specification is etimated to be ₹{int(round(op[0],-2))}''')
